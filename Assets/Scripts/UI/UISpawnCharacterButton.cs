...
public class UISpawnCharacterButton : MonoBehaviour
{
    ...
    public void SpawnPlayerCharacter(CharacterNames characterName)
    {
        ...
        // PhotonView는 PlayerCharacterGroup.SpawnCharacter에서 이미 등록됨
        // 여기서는 ViewID가 이미 할당되어 있어야 함
        if (characterPV == null)
        {
            Debug.LogError("[UISpawnCharacterButton] PhotonView를 찾을 수 없습니다!");
        ...
        if (nextSpawnTile != null)
        {
            ...
            {
                // PhotonView가 제대로 등록되었는지 확인
                if (characterPV.ViewID != 0)
                {
                    // 코루틴으로 지연하여 PhotonView 등록 시간 확보
                    StartCoroutine(DelayedSetSpawnPosition(characterPV, spawnPosition));
                }
                else
                {
                    // ViewID가 없으면 로컬에서만 설정
    ...
    // 코루틴 추가
    private IEnumerator DelayedSetSpawnPosition(PhotonView pv, Vector3 position)
    {
        // 한 프레임 대기하여 PhotonView 등록 시간 확보
        yield return null;

        if (pv != null && pv.ViewID != 0)
        {
            pv.RPC("SetSpawnPosition", RpcTarget.All, position);
        }
    }
